...
export class SpreadsheetServer {
    ...
    processRequest(req) {
        ...
        const region = new Region(sheet, req.limit?.rowStart ?? 1, req.limit?.rowStop ?? sheet.getLastRow() + 1, req.limit?.colStart ?? 1, req.limit?.colStop ?? sheet.getLastColumn() + 1, req.orientation ?? 'normal');
