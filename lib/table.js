...
export class Table {
    ...
    *indexValues() {
    ...
    async _save(changesOnly) {
        ...
        // Save to sheet
        const milestone = this.changes.current++;
        console.log('save milestone', milestone, 'rows', firstRow, lastRow);
        console.log(arr);
        if (!arr)
            return;
        console.log(arr.slice(0, lastRow - firstRow + 1)
            .map(r => r ? this.header.getRowValues(r) : undefined));
        console.log(JSON.stringify(arr.slice(0, lastRow - firstRow + 1)
            .map(r => r ? this.header.getRowValues(r) : undefined)));
