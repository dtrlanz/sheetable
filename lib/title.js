...
const titleProp = new MetaProperty('title');
const spreadProp = new MetaProperty('spread');
const restProp = new MetaProperty('rest');
...
export function getKeysWithTitles(obj, context, includeRest = true) {
    ...
    const enumerableProps = Object.entries(obj)
        .map(([key, value]) => [key, value, titleReader.get(obj, key)]);
    ...
    if (!found) {
        ...
        if (!found) {
            ...
            for (const key of toBeSpread) {
                ...
                if (typeof titlePropReader.get(obj, key) === 'object')
                    continue;
