...
const titleProp = new MetaProperty('title', undefined);
...
export function getKeysWithTitles(obj, context, includeRest = true) {
    ...
    const enumerableProps = Object.entries(obj)
        .map(([key, value]) => [key, value, mpReader.get(titleProp, key)]);
    ...
    const titleProps = mpReader.entries(titleProp)
        ...
        .map(([key, title]) => [key, obj[key], title]);
    const arr = [];
    for (let [key, value, title] of [...enumerableProps, ...titleProps]) {
        ...
        if (!isScalar(value)) {
            ...
            if (toBeSpread.has(key)) {
                ...
                if (title === undefined && typeof key === 'symbol')
...
export function getObjectPath(title, obj, context, includeRest = true) {
    if (title.length === 0)
        return [];
    ...
    if (!found) {
        ...
        if (!found) {
            ...
            for (const key of toBeSpread) {
                ...
                if (typeof mpReader.get(titleProp, key) === 'object')
                    continue;
...
export function getIndexTitles(ctor, context) {
    ...
    return keys.map(k => {
        ...
        if (Array.isArray(title))
            throw new Error('Properties with array titles are currently not allowed as indices');
        return title ?? k;
    });
}
