...
export const labelProp = new MetaProperty('label', undefined);
const spreadProp = new MetaProperty('spread', false);
const restProp = new MetaProperty('rest', false);
export const scalarProp = new MetaProperty('scalar', false);
...
export function label(label, ...rest) {
    if (rest.length === 0) {
...
export function getKeysWithLabels(obj, context, includeRest = true) {
    const ctor = Object.getPrototypeOf(obj).constructor;
    const sample = createFromEntries(ctor, []);
    ...
    if (restList.length > 1)
        throw new Error('only one member can be annoted with @rest');
    ...
    const enumerableProps = Object.entries(obj)
        .map(([key, value]) => [key, value, mpReader.get(labelProp, key)]);
    ...
    const labelProps = mpReader.entries(labelProp)
        ...
        .map(([key, label]) => [key, obj[key], label]);
    const arr = [];
    for (let [key, value, label] of [...enumerableProps, ...labelProps]) {
        ...
        if (!(key in sample || (includeRest && restKey === undefined)))
            continue;
        if (!(isScalar(value) || scalarLike.has(key))) {
            ...
            if (toBeSpread.has(key)) {
                ...
                if (label === undefined && typeof key === 'symbol')
...
export function getObjectPath(label, obj, context, includeRest = true) {
    if (label.length === 0)
        return [];
    ...
    if (!found) {
        ...
        if (!found) {
            ...
            for (const key of toBeSpread) {
                ...
                if (typeof mpReader.get(labelProp, key) === 'object')
                    continue;
...
export function getIndexLabels(ctor, context) {
    ...
    return keys.map(k => {
        ...
        if (Array.isArray(label))
            throw new Error('Properties with array labels are currently not allowed as indices');
        return label ?? k;
    });
}
