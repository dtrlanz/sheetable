 ...
 */
export function toSendable(value: Scalar): Sendable;
export function toSendable(value: Scalar[]): Sendable[];
export function toSendable(value: Scalar[][]): Sendable[][];
export function toSendable(value: (Scalar[] | undefined)[]): (Sendable[] | undefined)[];
export function toSendable(value: Scalar | (Scalar | Scalar[])[]): Sendable | (Sendable | Sendable[])[] {
    if (value && typeof value === 'object') {
        if (Array.isArray(value)) {
            return value.map(toSendable as any) as any;
        } else if (value instanceof Date) {
            return { date: value.toJSON() };
        }
        throw new Error(`cannot convert object types other than Array or Date: ${value}`);
    } else {
        return value as null | undefined | boolean | number | bigint | string | symbol;
    }
}
 ...
 */
export function fromSendable(value: Sendable): Scalar;
export function fromSendable(value: Sendable[]): Scalar[];
export function fromSendable(value: Sendable[][]): Scalar[][];
export function fromSendable(value: (Sendable[] | undefined)[]): (Scalar[] | undefined)[];
export function fromSendable(value: Sendable | (Sendable | Sendable[])[]): Scalar | (Scalar | Scalar[])[] {
    if (Array.isArray(value)) {
        return value.map(fromSendable as any) as any;
